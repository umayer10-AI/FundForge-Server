...
export const aiService = {
  ...
  async generateCampaignTitles(story: string) {
...
Return only the titles, one per line, numbered 1-5.`, {
  ...
  async improveStory(story: string) {
...
Return the improved story.`, {
  ...
  async generateSummary(story: string) {
...
Label each version clearly.`, {
  ...
  async suggestRewards(category: string) {
...
Make them specific to ${category} campaigns.`, {
  ...
  async suggestFundingGoal(category: string, story: string) {
...
4. Recommendations`, {
  ...
  async generateFAQ(campaignTitle: string, category: string, story: string) {
...
Provide a clear, helpful, and accurate answer based on how crowdfunding platforms work.`, {
  ...
  async analyzeCampaign(campaignData: { title: string; story: string; category: string; goal: number }) {
